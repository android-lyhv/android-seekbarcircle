package com.example.seekbarcircle;
...
public class SeekbarCircle extends View {
	...
	public SeekbarCircle(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		mIndicatorIcon = ContextCompat.getDrawable(context, R.drawable.ic_love);

		if (attrs != null) {
			// Attribute initialization
			final TypedArray typedArray = context.obtainStyledAttributes(attrs,
					R.styleable.SeekbarCircle, 0, 0);

			Drawable indicatorIcon = typedArray.getDrawable(R.styleable.SeekbarCircle_indicatorIcon);
			...
			mPoints = typedArray.getInteger(R.styleable.SeekbarCircle_points, mPoints);
			mMin = typedArray.getInteger(R.styleable.SeekbarCircle_min, mMin);
			mMax = typedArray.getInteger(R.styleable.SeekbarCircle_max, mMax);
			mStep = typedArray.getInteger(R.styleable.SeekbarCircle_step, mStep);

			mProgressWidth = (int) typedArray.getDimension(R.styleable.SeekbarCircle_progressWidth, mProgressWidth);
			progressColor = typedArray.getColor(R.styleable.SeekbarCircle_progressColor, progressColor);

			mArcWidth = (int) typedArray.getDimension(R.styleable.SeekbarCircle_arcWidth, mArcWidth);
			arcColor = typedArray.getColor(R.styleable.SeekbarCircle_arcColor, arcColor);

			mTextSize = (int) typedArray.getDimension(R.styleable.SeekbarCircle_textSize, mTextSize);
			textColor = typedArray.getColor(R.styleable.SeekbarCircle_textColor, textColor);

			mClockwise = typedArray.getBoolean(R.styleable.SeekbarCircle_clockwise,
					mClockwise);
			mEnabled = typedArray.getBoolean(R.styleable.SeekbarCircle_enabled, mEnabled);
			typedArray.recycle();
		}
...
//				System.out.printf("-- %d, %d, %f\n", progress, mPoints, mProgressSweep);
	...
	public interface OnSwagPointsChangeListener {
		 ...
		 *
		 * @param seekbarCircle The SwagPoints view whose value has changed
		 * @param points     The current point value.
		 * @param fromUser   True if the point change was triggered by the user.
		 */
		void onPointsChanged(SeekbarCircle seekbarCircle, int points, boolean fromUser);
